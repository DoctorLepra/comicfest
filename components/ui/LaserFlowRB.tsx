...
export const LaserFlowRB: React.FC<Props> = ({
  className,
  style,
  wispDensity = 1,
  dpr,
  ...
  useEffect(() => {
    ...
    const io = new IntersectionObserver(entries => { inViewRef.current = entries[0]?.isIntersecting ?? true; }, { threshold: 0 });
    io.observe(mount);
    ...
    const onMove = (ev: MouseEvent) => updateMouse(ev.clientX, ev.clientY);
    ...
    const animate = () => {
      ...
      const alpha = 1 - Math.exp(-cdt / Math.max(1e-3, mouseSmoothTime));
      ...
      if (now - lastFpsCheckRef.current > 750 && fpsSamplesRef.current.length > 0) {
        ...
        if (avg < 50) next = Math.max(0.6, currentDprRef.current * 0.85);
        else if (avg > 58 && currentDprRef.current < base) next = Math.min(base, currentDprRef.current * 1.1);
        if (Math.abs(next - currentDprRef.current) > 0.01 && now - lastDprChangeTime > 2000) {
  ...
  useEffect(() => {
    ...
    (u.uColor.value as THREE.Vector3).set(r, g, b);
