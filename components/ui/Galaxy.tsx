...
export default function Galaxy({
  ...
  glowIntensity = 0.3,
  saturation = 0.0,
  mouseRepulsion = false,   // repulsion OFF by default per user request
  repulsionStrength = 2,
  ...
  useEffect(() => {
    ...
    // Render at 0.5x resolution for performance — CSS upscales the canvas
    const dpr = Math.min(window.devicePixelRatio * 0.5, 0.75);
  ...
  return (
    <div
      ref={ctnDom}
      className={`galaxy-container${className ? " " + className : ""}`}
    />
  );
}
